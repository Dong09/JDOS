...
; BaseOfStack         equ     0100h       ; Boot状态下堆栈基地址(栈底, 从这个位置向低地址生长)
; PageDirBase         equ     100000h     ;页目录开始的地方： 1M
...
;     mov	edx, 0534D4150h
;     int	15h
;     jc	LABEL_MEM_CHK_FAIL		;当CF位为1时 跳出循环
;     add	di, 20
;     inc	dword [_dwMCRNumber]	;每循环一次+1 代表ARDS的个数
;     cmp	ebx, 0
;     jne	.loop					;当ebx被置为0，跳出循环
;     jmp	LABEL_MEM_CHK_OK
; LABEL_MEM_CHK_FAIL:				;检查内存大小失败
;     mov	dword [_dwMCRNumber], 0	;将ARDS个数重置为1
; LABEL_MEM_CHK_OK:				;检查正常
...
; 	mov	di, OffsetOfKernelFile	                ; es:di -> BaseOfKernelFile:0100 = BaseOfKernelFile*10h+100
; 	cld
; 	mov	dx, 10h
...
; 	cmp	al, byte [es:di]
; 	jz	LABEL_GO_ON
; 	jmp	LABEL_DIFFERENT		                ; 只要发现不一样的字符就表明本 DirectoryEntry 不是我们要找的 KERNEL.BIN
; LABEL_GO_ON:
; 	inc	di
...
;     add    cx, ax
;     add    cx, DeltaSectorNo    ; 这句完成时 cl 里面变成 KERNEL.BIN 的起始扇区号 (从 0 开始数的序号)
;     mov    ax, BaseOfKernelFile
;     mov    es, ax            ; es <- BaseOfKernelFile
;     mov    bx, OffsetOfKernelFile    ; bx <- OffsetOfKernelFile    于是, es:bx = BaseOfKernelFile:OffsetOfKernelFile = BaseOfKernelFile * 10h + OffsetOfKERNEL
...
; ; *****************************************************************************************************
;     ; jmp    BaseOfKernelFile:OffsetOfKernelFile    ; 这一句正式跳转到已加载到内存中的 KERNEL.BIN 的开始处
;                         ; 开始执行 KERNEL.BIN 的代码
;                         ; Boot Sector 的使命到此结束
; ; *****************************************************************************************************
...
;     test    edx, edx        ;将两个数进行逻辑与运算，不保存结果，只修改标志位
;     jz	.no_remainder   ;如果z标志位为0，说明整除
...
; .1:
;     stosd               ;mov  dword ptr es:[edi],eax    add edi,4
...
;; 变量
_dwMCRNumber:               dd 0    ; Memory Check Result
_dwDispPos:                 dd (80 * 7 + 0) * 2    ; 屏幕第 6 行, 第 0 列。
_dwMemSize:                 dd 0
_ARDStruct:    ; Address Range Descriptor Structure
    _dwBaseAddrLow:         dd    0
    _dwBaseAddrHigh:        dd    0
